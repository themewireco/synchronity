...
const php = readFileSync(join(src, 'agentmesh-woocommerce.php'), 'utf8');
...
const slugDir = join(stage, 'agentmesh-woocommerce');
cpSync(src, slugDir, { recursive: true });

const out = join(root, `synchronity-woocommerce-v${version}.zip`);
rmSync(out, { force: true });
execSync(`cd "${stage}" && zip -r -q "${out}" agentmesh-woocommerce`, { stdio: 'inherit' });
