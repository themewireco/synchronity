...
export function createMCPServer(config: {
  ...
  // These instructions describe what the server covers and how its tools fit
  // together. They deliberately avoid directing the assistant's behaviour
  // outside this server (which other tools to use, when to ask the user for
  // permission) — that is both a directory-policy requirement and the reason
  // the host, not the server, owns those decisions.
  const SERVER_INSTRUCTIONS = [
    ...
    'SITE IDS: every tool except list_sites takes a site_id. list_sites returns the connected stores with their IDs and matches partial or approximate store names.',
    'BROWSING vs SEARCHING: search_products with no query returns the store\'s catalog, which is what an open-ended request for what a store carries needs. Passing a query searches for a specific product name or keyword. Price, category, and stock filters are applied server-side and change which products come back.',
    'CART: add_to_cart adds a line, set_cart_quantity sets a line\'s absolute quantity (0 removes it), and remove_from_cart drops a line. get_cart returns the authoritative totals including tax, shipping, and discounts, which is what checkout charges.',
    'RESUMING: get_active_cart(site_id) returns the buyer\'s in-progress cart for a store, so an interrupted or continued shopping session can pick up its existing items instead of starting over.',
    'CHECKOUT: checkout and payment complete only with the buyer\'s explicit approval, given as a one-time code emailed to them and entered by them. An order is complete when the payment status is paid or the order is processing or completed; any other status means it is not.',
  ].join('\n');
